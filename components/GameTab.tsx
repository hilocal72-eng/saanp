      ...
      for (let col = 0; col < 10; col++) {
        ...
        cells.push(
          <div key={cellNum} className="relative flex items-center justify-center border-[0.5px] border-black/10 bg-white" style={{ width: '10%', aspectRatio: '1/1' }}>
            <span className="absolute top-0.5 left-1 text-[8px] sm:text-[10px] font-black text-slate-900 select-none opacity-80">{cellNum}</span>
    ...
    <div className="flex flex-col h-[100dvh] bg-slate-950 text-white overflow-hidden pb-48">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col items-center">
        <div className="flex justify-center items-center gap-10 mb-4">
           <div className={`p-4 rounded-2xl border-2 flex flex-col items-center shadow-lg transition-all ${game.turn === 'host' ? 'bg-indigo-600/20 border-indigo-500 scale-105' : 'bg-slate-900/40 border-transparent opacity-30'}`}>
              <span className="text-2xl font-black text-white">{game.hostLastDice || '-'}</span>
              <span className="text-[8px] font-black mt-1 uppercase text-indigo-400 tracking-widest">HOST</span>
           </div>
           <div className={`p-4 rounded-2xl border-2 flex flex-col items-center shadow-lg transition-all ${game.turn === 'guest' ? 'bg-emerald-600/20 border-emerald-500 scale-105' : 'bg-slate-900/40 border-transparent opacity-30'}`}>
              <span className="text-2xl font-black text-white">{game.guestLastDice || '-'}</span>
              <span className="text-[8px] font-black mt-1 uppercase text-emerald-400 tracking-widest">GUEST</span>
           </div>
        </div>
        ...
        <div className="w-full max-w-[95vw] mx-auto bg-white rounded-xl border-[6px] border-slate-900 shadow-2xl relative aspect-square overflow-hidden">
          ...
          <div className="absolute inset-0 z-[60] pointer-events-none">
            <div className="absolute w-5 h-5 bg-indigo-600 rounded-full border-2 border-white shadow-xl transition-all duration-300 ease-in-out animate-bounce" style={{ left: `${hostCoords.x}%`, top: `${hostCoords.y}%`, transform: 'translate(-50%, -50%)' }} />
            <div className="absolute w-5 h-5 bg-emerald-600 rounded-full border-2 border-white shadow-xl transition-all duration-300 ease-in-out animate-bounce" style={{ left: `${guestCoords.x}%`, top: `${guestCoords.y}%`, transform: 'translate(-50%, -50%)' }} />
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-[110px] left-0 right-0 px-6 z-[110] flex justify-center">
        {game.winner ? (
          <div className={`w-full max-w-xs p-6 rounded-[2rem] text-center shadow-2xl border animate-in slide-in-from-bottom-10 ${iWon ? 'bg-indigo-600 border-indigo-400' : 'bg-slate-900 border-rose-500/30'}`}>
            <h2 className="text-2xl font-black uppercase mb-4">{iWon ? 'VICTORY' : 'DEFEAT'}</h2>
            <button onClick={confirmLeave} className="px-8 py-3 bg-white text-black rounded-xl font-black text-xs uppercase">BACK TO LOBBY</button>
          </div>
        ) : (
          <button 
          ...
          >
            ...
            {isMyTurn && game.guestId && !rolling && !isAnimating && (
              <div className="absolute -top-1 -right-1 w-6 h-6 bg-white rounded-full flex items-center justify-center border-2 border-indigo-600">
